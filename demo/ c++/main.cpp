...
bool g_isScanning = false;              // 是否正在扫描磁盘
...
LRESULT CALLBACK HotKeyDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowExW(0, L"STATIC", L"请按下新的热键组合：",
...
void DoSearch() {
    ...
    if (selFilter != CB_ERR) {
        wchar_t buf[128] = {0};
        SendMessageW(g_hFilterCombo, CB_GETLBTEXT, selFilter, (LPARAM)buf);
        std::string filterName = Utf16ToUtf8(buf);
    ...
    switch (state) {
        case 0:
            if (g_isScanning || g_isLoading) {
                g_isScanning = false;
                g_isLoading = false;
                _snwprintf(status, 256, L"就绪 - %d 个文件", g_fileCount);
        ...
        case 1:
            ...
            break;
        case 2: SetWindowTextW(g_hStatusBar, L"正在保存数据库..."); break;
        case 3:
            g_isScanning = true;
...
void InitUI(HWND hwnd) {
    ...
    g_hSearchEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
        WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL, 50, y + 2, 420, h - 4, hwnd, (HMENU)ID_SEARCH_EDIT, NULL, NULL);
    SendMessageW(g_hSearchEdit, WM_SETFONT, (WPARAM)g_hFont, TRUE);
    SendMessageW(g_hSearchEdit, EM_SETCUEBANNER, TRUE, (LPARAM)L"输入关键词搜索文件...Esc:清空");
...
void OnGetDispInfo(NMLVDISPINFOW* pDispInfo) {
    if (!g_result || g_searchFingerprint < 0) return;
...
LRESULT OnCustomDraw(LPNMLVCUSTOMDRAW pcd) {
    switch (pcd->nmcd.dwDrawStage) {
        ...
        case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
            if (pcd->iSubItem != 0 || !g_result || g_searchFingerprint < 0)
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_COMMAND:
            ...
            } else if (HIWORD(wParam) == EN_CHANGE && LOWORD(wParam) == ID_SEARCH_EDIT) {
        ...
        case WM_LOAD_COMPLETE: {
            ...
            wchar_t status[256];
            _snwprintf(status, 256, L"数据库加载完成，正在搜索...");
            SetWindowTextW(g_hStatusBar, status);
            DoSearch();
            break;
        }
        ...
        case WM_SCAN_COMPLETE: {
            ...
            wchar_t status[256];
            _snwprintf(status, 256, L"磁盘扫描完成，正在搜索...");
            SetWindowTextW(g_hStatusBar, status);
        ...
        case WM_SEARCH_COMPLETE: {
            ...
            wchar_t status[256];
            _snwprintf(status, 256, L"找到 %d 个结果 (%d ms)", data->resultCount, data->elapsedMs);
            SetWindowTextW(g_hStatusBar, status);
            delete data;
            break;
        }
